...
{ 
  ...
  { 
    ...
    // double t_= (pl_orig.points[i].timestamp - int(pl_orig.points[i].timestamp))*pow(10,6);
    double t_= (pl_orig.points[i].timestamp - pl_orig.points[0].timestamp) * pow(10,3);
    if (i % point_filter_num != 0) continue;
    double range = pl_orig.points[i].x * pl_orig.points[i].x + pl_orig.points[i].y * pl_orig.points[i].y + pl_orig.points[i].z * pl_orig.points[i].z;
    ...
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    // added_pt.curvature = t_ * time_unit_scale; // curvature unit: ms 
    added_pt.curvature = t_ ; // curvature unit: ms 
...
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, vector<orgtype> &types)
{
  ...
  uint head = 0;
  std::cout << types[head].range << " " << blind << std::endl;
